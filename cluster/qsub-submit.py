...
mem_mb = resources.get(
    ...
    cluster.get(
        ...
        int(cluster_config['default_mem_mb'])  # otherwise, use default value
    )
)
...
log_dir = cluster.get("logdir", cluster_config['default_cluster_logdir'])
...
queue = cluster.get("queue", cluster_config['default_queue'])
...
if (
        threads >= int(cluster_config['reserve_min_threads'])
        or
        mem_mb >= int(cluster_config['reserve_min_mem_mb'])
):
    resources_cmd += " -R yes"
